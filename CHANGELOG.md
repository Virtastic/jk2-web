...
- **Docker build that works from a clean clone.** `docker build -t jk2-web .` now compiles the
...
- The deploy staged build products with `rsync --delete`, which replaced the whole `play/jk2/`
...
- **The Cloud Locker rejected every legitimate upload** whenever the editions manifest was
  ...
  with no `.pk3` at all.
- Documentation that contradicted the shipped code: `docs/DATA.md` still told people to add a
  `productid.txt` marker to `base/` and set `__JK2_GAMEDIR`. That instruction was retracted — the
  file is a key checked against a scrambled table, not a marker, and the retail data already ships
  it inside a pak.
...
First public release: **Jedi Knight II: Jedi Outcast** single-player, running in the browser from
Raven's original GPL sources.

### The port

- The complete single-player campaign runs end to end: **26/26 maps** load and reach gameplay
...
- `launcher.html` — choose where game data comes from: Cloud Locker, data hosted by the site, the
  free demo mission, or your own install via the folder picker.
