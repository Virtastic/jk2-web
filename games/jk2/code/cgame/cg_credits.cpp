		...
		else
		{
			...
			if (CreditData.CreditLines.size())
			{
				...
				for (CreditLines_t::iterator it = CreditData.CreditLines.begin(); it != CreditData.CreditLines.end(); bEraseOccured ? it : ++it)
				{
					...
					// idTech3-web: MSVC6's windows.h max() macro reached this TU via /YX
					// auto-PCH. std::max needs both args the same type; cast to match the
					// macro's int result exactly.
