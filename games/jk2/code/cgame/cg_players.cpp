...
// idTech3-web: MSVC6 /YX auto-PCH leaked these decls into this TU; include explicitly
...
//	const int iVariantCap = 2;	// test
	...
	{
		char *p = (char *)strchr(ppsTable[iEntryNum],'.');	// idTech3-web: C++ strchr returns const char* here; p is reused on a mutable buffer below
...
static void CG_ForceElectrocution( centity_t *cent, const vec3_t origin, vec3_t tempAngles )
{
	...
	if ( tr.fraction < 1.0f || Q_flrandom() > 0.94f )
	{
		FX_AddElectricity( fxOrg, tr.endpos,
			1.5f, 4.0f, 0.0f, 
			1.0f, 0.5f, 0.0f,
			rgb, rgb, 0.0f,
			5.5f, Q_flrandom() * 50 + 100, cgs.media.boltShader, FX_ALPHA_LINEAR | FX_SIZE_LINEAR | FX_BRANCH | FX_GROW | FX_TAPER );
	}
}
/*
===============
