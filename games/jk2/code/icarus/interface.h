...
// idTech3-web: wrapper restoring the MSVC fn-ptr default arg
#ifndef IDT3_IREADSAVE_FP_DEFINED
#define IDT3_IREADSAVE_FP_DEFINED
struct idt3_IReadSaveData_fp {
	typedef int (*fn_t)( unsigned long chid, void *address, int length, void **addressptr );
	fn_t fn;
	idt3_IReadSaveData_fp &operator=( fn_t f ) { fn = f; return *this; }
	operator fn_t() const { return fn; }
	int operator()( unsigned long chid, void *address, int length, void **addressptr = 0 ) const
	{ return fn( chid, address, length, addressptr ); }
};
#endif
...
typedef struct interface_export_s
{
	...
	idt3_IReadSaveData_fp I_ReadSaveData; //idTech3-web
