...
inline int Q_irand(int min, int max) {
	...
	// scaling and returns wildly out-of-range values (Q_irand(0,6) -> 33274, which
	// then indexes ambientSet_t::subWaves[] far out of bounds -> "handle out of range").
	// Mask to 15 bits so the original arithmetic behaves exactly as on Win32.
