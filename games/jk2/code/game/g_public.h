...
Ghoul2 Insert End
*/
// idTech3-web: MSVC allowed default args on function-pointer params; clang rejects them.
// idt3_trace_fp is layout-identical to the raw pointer; operator() restores the defaults.
#ifndef IDT3_TRACE_FP_DEFINED
#define IDT3_TRACE_FP_DEFINED
struct idt3_trace_fp {
	typedef void (*fn_t)( trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end,
	...
	void operator()( trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end,
			const int passEntityNum, const int contentmask, const EG2_Collision eG2TraceType = (EG2_Collision)0, const int useLod = 0 ) const
	{ fn( results, start, mins, maxs, end, passEntityNum, contentmask, eG2TraceType, useLod ); }
};
#endif
// idTech3-web: wrappers restoring MSVC fn-ptr default args (see idt3_trace_fp note).
...
typedef struct {
	...
	idt3_trace_fp trace; //idTech3-web: was fn-ptr with MSVC default args
