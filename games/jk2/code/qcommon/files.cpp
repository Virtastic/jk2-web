...
// idTech3-web: the constant above is, by its own comment, a build-time value that
// tracked whatever demo pk3 Raven had last built in-house. It does NOT describe the
// demo that was actually released to the public (jk2demo.exe, 2002-04-04, whose
// GameData/demo/assets0.pk3 is 64001989 bytes / 2168 entries), so this GPL retail
// source rejects the only freely-available JK2 data with "Corrupted pk3".
//
// The value below is that public demo pak's checksum, computed by the engine's own
// algorithm (unzip central-directory CRCs of entries with uncompressed_size > 0 ->
// Com_BlockChecksum -> MD4, XOR-folded). It was independently reimplemented in
// Python and agrees with what this engine computes, which is also a check on our
// wasm port: unzip + MD4 + the checksum path all produce the identical value.
//
// This does not weaken the restriction, it retargets it: demo mode still accepts
// exactly ONE known pak and still refuses retail paks dropped into demo/. The
// original constant is kept and still honoured.
#define	DEMO_PAK_CHECKSUM_PUBLIC	1431467275u
...
static void FS_SetRestrictions( void ) {
	...
	for ( path = fs_searchpaths ; path ; path = path->next ) {
		if ( path->pack ) {
			...
			// idTech3-web: also accept the PUBLIC demo pak (see DEMO_PAK_CHECKSUM_PUBLIC).
			// Still exactly two known paks; anything else — including a retail pak
			// dropped into demo/ — is still refused.
			if ( (path->pack->checksum ^ 0x84268436u) != (DEMO_PAK_CHECKSUM ^ 0x84268436u)
			  && (path->pack->checksum ^ 0x84268436u) != (DEMO_PAK_CHECKSUM_PUBLIC ^ 0x84268436u) ) {
