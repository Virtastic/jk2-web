...
// idTech3-web: WebGL1 client-array draws accept ONLY GL_UNSIGNED_SHORT —
// emscripten's glDrawElements asserts exactly that when no element array buffer is
// bound (`assert(type == GLctx.UNSIGNED_SHORT)`), so a 32-bit index type aborts on
// the first real draw. SHADER_MAX_VERTEXES stays below 65536, so this is lossless.
// (JKA's tree carries the same split natively, for the Xbox build.)
