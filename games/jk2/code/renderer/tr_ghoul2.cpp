...
void RB_SurfaceGhoul( CRenderableSurface *surf ) {
	...
	// idTech3-web: must follow glIndex_t, not a bare `unsigned int *`. Under wasm
	// glIndex_t is 16-bit (WebGL1 client-array draws take only GL_UNSIGNED_SHORT), so
	// a 32-bit alias would write two index slots per entry and shred the mesh. Identical
	// to `unsigned int *` on desktop, where glIndex_t IS unsigned int. Matches JKA's tree.
