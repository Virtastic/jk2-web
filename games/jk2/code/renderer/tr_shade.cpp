...
static void ProjectDlightTexture( void ) {
	...
	MAC_STATIC float	texCoordsArray[SHADER_MAX_VERTEXES][2];
	byte	colorArray[SHADER_MAX_VERTEXES][4];
	// idTech3-web: R_DrawElements takes const glIndex_t *, which is 16-bit under wasm;
	// these values are vertex indices < SHADER_MAX_VERTEXES (< 65536) so it is lossless.
	// glIndex_t IS unsigned int on desktop, so this is a no-op there. Matches JKA's tree.
