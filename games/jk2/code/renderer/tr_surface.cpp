...
static int f_count; // idTech3-web: implicit-int
...
void RB_SurfaceFace( srfSurfaceFace_t *surf ) {
	int			i, k;
	unsigned	*indices;
	// idTech3-web: this MUST follow glIndex_t, not be a bare `unsigned *`. Under
	// __EMSCRIPTEN__ glIndex_t is 16-bit (WebGL1 client-array draws only accept
	// GL_UNSIGNED_SHORT), so a 32-bit alias writes two index slots per entry and
	// shreds all world geometry into stray triangles. JKA's own tree already
	// declares this as glIndex_t* — JK2's does not.
...
//do test
	...
	// idTech3-web: WebGL1 cannot glReadPixels(GL_DEPTH_COMPONENT) — INVALID_ENUM once per
	// flare per frame. The occlusion z-test is impossible; treat the flare as visible (it
	// still fades by view angle/distance in RB_SurfaceFlare).
	(void)depth; (void)screenZ; (void)visible;
	return qtrue;
#else
