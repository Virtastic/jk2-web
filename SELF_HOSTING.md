...
```sh
git clone https://github.com/Virtastic/jk2-web && cd jk2-web
...
Grab `jk2-web-<tag>.zip` from [Releases](https://github.com/Virtastic/jk2-web/releases). It
contains the prebuilt engine plus the pages and the dev server:

```sh
unzip jk2-web-*.zip -d jk2-web && cd jk2-web
python3 server.py                      # http://localhost:8793
```
...
the safest posture, and the one <https://jk2.virtastic.app> uses alongside the demo.

### The free demo (recommended, and legal to redistribute)

The official Jedi Outcast demo pak is freely redistributable. Put it at `demo/assets0.pk3` next to
...
Put a **complete** retail set — `assets0.pk3`, `assets1.pk3`, `assets2.pk3`, `assets5.pk3` — in
`base/`. There is **no marker file to create**: `productid.txt` is not a marker but a key checked
against a scrambled table in the engine, and the retail data already ships it inside `assets0.pk3`.
...
> Retail Jedi Outcast data is commercial and **not redistributable**. Host it only where you have
...
> jk2 is single-threaded and does not strictly *need* cross-origin isolation to run. The headers
...
(`cloud/jk2-editions.json`) so the locker cannot be used as general file storage.
...
corresponding source: this repository, or the `jk2-web-src-<tag>.tar.gz` from the matching release.
