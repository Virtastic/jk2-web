# Cloudflare DNS for jk2.virtastic.app (the Jedi Knight II: Jedi Outcast preview on the shared OVH VPS).
...
resource "cloudflare_record" "jk2" {
  ...
  proxied = true
  ttl     = 1
  comment = "Jedi Knight II: Jedi Outcast preview on the shared OVH VPS (managed by terraform)"
}
