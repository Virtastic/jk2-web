...
`games/jk2/` holds Raven's original GPL drop, imported in a single commit. Every browser adaptation
is a separate, reviewable commit on top of it. **The diff of `games/jk2` against its import commit
...
shared/wasm-build/build-jk2.sh && shared/wasm-build/build-jk2-modules.sh
...
node shared/wasm-build/verify-icarus-affect.mjs 8793   # ICARUS regression guard
```
