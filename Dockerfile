...
# Jedi Knight II: Jedi Outcast — thin static image for jk2.virtastic.app.
# The wasm/js are gitignored build products (built with the pinned Homebrew emcc 6.0.1) and are
# staged into play/jk2/ in the build context by the deploy workflow (rsync'd onto the runner),
...
LABEL org.opencontainers.image.title="jk2" \
      org.opencontainers.image.description="Jedi Knight II: Jedi Outcast — WebAssembly port (preview)" \
