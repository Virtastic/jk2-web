...
# Windows one before handing it to a native .exe. emcc IS a native .exe there, and that
# rewrite mangles compiler *values* — -DPATH_SEP='/' becomes the MSYS root directory and
# -DIDT3_FSROOT="/jk2" turns into "C:/Program Files/Git/jk2". Exclude the flags whose
# values are engine paths, not host paths; -I/-L/-o/-include still convert, as they must.
case "$(uname -s)" in
  MINGW*|MSYS*|CYGWIN*)
