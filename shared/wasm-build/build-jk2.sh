...
# ---- full compile + link -------------------------------------------------
# Only encryption/ is excluded now (net-only, and unused by SP). The list that used to sit
# here -- mp3code, client/snd_*, cl_mp3, cl_cin -- described the first link, when sound and
# cinematics were stubbed out in sys_jk_snd.cpp because JK2's audio path is OpenAL + Creative
# EAX (client/openal, client/eax) and does not build under emscripten. All of it is compiled
# for real today; only the OpenAL/EAX back end itself is replaced.
...
# L3.h's little-endian tables. The real RoQ player (client/cl_cin.cpp) is compiled too --
# assets0.pk3 carries 15 videos (jk0101..jk09, the Mon Mothma briefing set, openinglogos,
