...
void GLimp_Init( void ) {
	...
	// idTech3-web: GLW_InitExtensions() (win32/win_glimp.cpp:1067-1108) fills the remaining
	// glConfig capability fields. That function lives in the excluded win32 GL layer, so both
	// of these stayed qfalse from the memset above, and the engine reported anisotropic
	// filtering and texenv add as unavailable while the context supported both.
	//
	// DO NOT copy jka-web's version of this block. JK2's glconfig_t has NO
	// maxTextureFilterAnisotropy field at all — textureFilterAnisotropicAvailable is a plain
	// boolean gate and tr_image.cpp applies a HARDCODED 2.0f (tr_image.cpp:127-131 and
	// 710-711). There is consequently no cvar clamp to defeat here, which is the whole reason
	// JKA's symptom (its "+set r_ext_texture_filter_anisotropic 8" being rewritten to 0) has
	// no JK2 equivalent. Both cvars default to "1" (tr_init.cpp:960-961).
	...
	if ( glConfig.extensions_string &&
		...
		IDT3_RI_CVARSET( "r_ext_texture_filter_anisotropic_avail", "1" );
	} else {
		IDT3_RI_CVARSET( "r_ext_texture_filter_anisotropic_avail", "0" );
	}

	// GL_EXT_texture_env_add. The original gates on the extension string AND the cvar;
	// emscripten's GL emulation does not advertise the extension, but its TexEnv combiner
	// does implement GL_ADD (libglemu.js has explicit GL_ADD cases), so honour the cvar.
	// Without this, CollapseMultitexture() (tr_shader.cpp:2452) refuses to fold an additive
	// two-stage shader into one multitexture pass and draws it as two passes instead.
	glConfig.textureEnvAddAvailable = r_ext_texture_env_add->integer ? qtrue : qfalse;
