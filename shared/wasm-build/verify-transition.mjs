...
const MAP_A = process.argv[3] || 'kejim_post';
const MAP_B = process.argv[4] || 'kejim_base';
const MAP_C = process.argv[5] || 'artus_mine';
...
`});
// Uncapped log capture. index.html funnels Module.print into a ring capped at 2000 lines that
// SHIFTS, so anything early -- a map's arrival script, in particular -- is gone by the time a probe
// dumps it. That is how a later script's ordinary timestamps got read as a clock that had not
// reset, and why "no CGCam_Enable was logged" could never be trusted. Wrap Module.print as soon as
// it exists and keep every line.
await S('Page.addScriptToEvaluateOnNewDocument', { source: `
  window.__allLog = [];
  (function hook(){
    try {
      if (typeof Module === 'object' && Module && !Module.__idt3Hooked) {
        for (const k of ['print','printErr']) {
          ...
          if (typeof orig === 'function') {
            Module[k] = function(t){ try { window.__allLog.push(String(t)); } catch(e){} return orig.apply(this, arguments); };
          }
        }
        Module.__idt3Hooked = true;
        return;
      }
    } catch (e) {}
    setTimeout(hook, 10);
...
// same-map reloads are.
{
  ...
  // Plain observation, no commands issued: does the player ever leave the origin, and when?
  // Everything else here has proved able to mislead -- `cam_disable` "fixing" it may simply have
  // coincided with the player finally spawning -- so this measures one thing and interferes with
  // nothing.
  let placed = null, placedAt = -1;
  const t0 = Date.now();
  for (let round = 0; round < 90; round++) {
    placed = await posOf();
    if (!atOrigin(placed)) { placedAt = Math.round((Date.now() - t0) / 1000); break; }
    await sleep(2000);
  }
  console.log(`  left origin after     : ${placedAt < 0 ? 'NEVER (>' + Math.round((Date.now()-t0)/1000) + 's)' : placedAt + 's'}`);
  // Dump the UNCAPPED log to a file so the arrival sequence can be read in full.
  {
    const all = await S('Runtime.evaluate', { returnByValue: true, expression: 'JSON.stringify(window.__allLog||[])' });
    let arr = [];
    try { arr = JSON.parse(all.result.value || '[]'); } catch {}
    const fsmod = await import('node:fs');
    const out = process.env.LOGFILE || 'idt3-full.log';
    fsmod.writeFileSync(out, arr.join(String.fromCharCode(10)));
    console.log(`  full log lines        : ${arr.length}  -> ${out}`);
    const cam = arr.filter(l => /camera/i.test(l));
    console.log(`  lines mentioning camera: ${cam.length}`);
    for (const l of cam.slice(0, 12)) console.log('    ' + String(l).trim());
  }
  console.log(`  player placed at      : ${placed ? placed.join(' ') : '(none)'}${atOrigin(placed) ? '  <- STILL AT ORIGIN' : ''}`);
  // Capture the script trace HERE, not at the end of the run: the log ring is capped and by the
  // time the probe finishes its settle loops the arrival script's lines have scrolled out, leaving
  // only much later scripts (which is how a perfectly ordinary level.time of 409s got mistaken for
  // a clock that had not reset).
  if (process.env.EXTRA_ARGS) {
    const early = (await ringText()).split(String.fromCharCode(10)).map(x => x.trim()).filter(Boolean);
    // Show what ran AFTER the new map's InitGame, not the tail of the whole ring -- the previous
    // map's still-running scripts otherwise dominate.
    let mk = -1;
    for (let z = early.length - 1; z >= 0; z--) if (early[z].includes('IDT3TIME')) { mk = z; break; }
    const after = mk >= 0 ? early.slice(mk) : early;
    const rel = after.filter(l => /IDT3CAM|IDT3TIME|introscript|camera/i.test(l));
    console.log('  --- new map, script trace after InitGame ---');
    for (const l of rel.slice(0, 40)) console.log('    ' + l);
  }
  ...
  if (!(dd > 24)) {
    ...
    {
      const tt = (await ringText()).split(String.fromCharCode(10)).filter(x => x.includes('IDT3TIME'));
      console.log('  --- level.time per map ---');
      for (const l of tt) console.log('    ' + l.trim());
    }
    if (process.env.EXTRA_ARGS) {
