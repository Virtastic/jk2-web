...
// Measured on kejim_post with the fix REMOVED: 28 skipped blocks, then 28 again, then 0.
// It is a race, so it is intermittent. That has two consequences worth being clear about:
...
// With the fix in place it has not been observed to fire at all.
//
...
const MAP  = process.argv[3] || 'kejim_post';
