#!/usr/bin/env bash
# Build the jk2 test image ON the build server (run there, in the synced source dir).
...
SRC="${SRC:-$HOME/jk2-src}"
TAG="${TAG:-jk2:test}"
cd "$SRC"

[ -f Dockerfile.test ] || { echo "FATAL: no Dockerfile.test in $SRC (sync first)"; exit 1; }
[ -d games/jk2/code ]  || { echo "FATAL: games/jk2/code missing in $SRC (sync problem)"; exit 1; }
