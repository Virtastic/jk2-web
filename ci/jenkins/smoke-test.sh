...
echo "==> jk2 serving contract: $BASE"

# 1. Cross-origin isolation headers. jk2 is single-threaded so it runs without them, but the vhost
...
B="$(get "$BASE/")"
has 'Jedi Knight' "$B" && pass "root serves the launcher" || fail "launcher at /" "root did not return the jk2 launcher"

# 3. The engine + game module are present and typed correctly. jk2 serves them at the web root.
[ "$(code "$BASE/jk2.js")" = 200 ]      && pass "engine jk2.js served"     || fail "jk2.js" "engine loader missing"
[ "$(code "$BASE/jk2.wasm")" = 200 ]    && pass "engine jk2.wasm served"   || fail "jk2.wasm" "engine wasm missing"
[ "$(code "$BASE/qagame.wasm")" = 200 ] && pass "game module qagame.wasm served" || fail "qagame.wasm" "SIDE_MODULE missing"
has '^content-type: *application/wasm' "$(hdrs "$BASE/jk2.wasm")" && pass "jk2.wasm is application/wasm" || fail "wasm mime" "wrong Content-Type"
