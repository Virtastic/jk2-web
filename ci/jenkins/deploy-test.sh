...
# Run ON the build server (it holds jk2:test and can ssh the test host). No registry:
...
SSH_KEY="${SSH_KEY:-$HOME/.ssh/jk2-deploy}"
TAG="${TAG:-jk2:test}"
NAME="${NAME:-jk2-test}"
PORT="${PORT:-8082}"
...
# The ingress in front routes jk2.dev.virtastic.app -> TEST_HOST:PORT (TLS/SNI there).
$SSH "$TEST_HOST" "
  set -e
