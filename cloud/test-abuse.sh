...
c=$(code -X PUT --data-binary @"$TMP/chunkA.bin" -H 'x-jk2-chunk-offset: 0' -H 'x-jk2-total-size: 1000000' "$B$U")
...
c=$(code -X PUT --data-binary @"$TMP/chunkB.bin" -H 'x-jk2-chunk-offset: 999' -H 'x-jk2-total-size: 1000000' "$B$U")
[ "$c" = 409 ] && pass "out-of-order chunk rejected (409)" || fail "chunk ordering" "got $c"
c=$(code -X PUT --data-binary @"$TMP/chunkB.bin" -H 'x-jk2-chunk-offset: 600000' -H 'x-jk2-total-size: 1000000' "$B$U")
...
c=$(code -X PUT --data-binary @"$TMP/chunkA.bin" -H 'x-jk2-chunk-offset: 0' -H 'x-jk2-total-size: 99999999' "$B$U")
...
# --- "is this actually JK2 data?" (second instance: verification ON, real-world limits) ---------
...
# assets5.pk3 is a real retail file: 550829 bytes, md5 b7c34a06029798e178c075216ea37a9d (see cloud/*-editions.json).
r=$(v2 '{"path":"assets5.pk3","size":550829,"md5":"b7c34a06029798e178c075216ea37a9d"}')
echo "$r" | grep -q '"verified":true' && pass "tier 1: exact hash match is marked verified" || fail "tier 1" "not verified: $r"
r=$(v2 '{"path":"assets5.pk3","size":558000}')
...
r=$(v2 '{"path":"assets5.pk3","size":550829,"md5":"'"$(printf 'f%.0s' $(seq 1 32))"'"}')
echo "$r" | grep -q '"verified":false' && pass "a wrong hash falls through to the size tier, never upgrades" || fail "hash trust" "$r"
r=$(v2 '{"path":"assets5.pk3","size":9000000}')
echo "$r" | grep -q 'not a recognized' && pass "size wildly outside tolerance is refused" || fail "tolerance bound" "$r"
c=$(v2 '{"path":"totally-made-up.pk3","size":550829}' | grep -c 'not a recognized'); [ "$c" = 1 ] \
  && pass "unknown filename refused (no edition ships it)" || fail "unknown file" "accepted"
c=$(v2 '{"manifest":{"files":[{"path":"pirate-movie.pk3","size":550829}]}}' | grep -c 'not a recognized')
[ "$c" = 1 ] && pass "manifest listing a non-JK2 filename refused" || fail "manifest allowlist" "accepted"
...
h=$(curl -s -D - -o /dev/null -H "cookie: jk2_session=$OLDTOK" "$B/api/me")
echo "$h" | grep -qi '^set-cookie: jk2_session=' && pass "old-but-valid session is renewed (fresh cookie)" \
  || fail "sliding renewal" "no Set-Cookie on an aging token"
FRESH=$(curl -s -c - -o /dev/null "$B/api/auth/dev/login?uid=fresh2" | grep jk2_session | awk "{print \$NF}")
h2=$(curl -s -D - -o /dev/null -H "cookie: jk2_session=$FRESH" "$B/api/me")
echo "$h2" | grep -qi '^set-cookie: jk2_session=' && fail "renewal churn" "fresh token was reissued anyway" \
