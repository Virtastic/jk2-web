// jk2-cloud - OAuth login + game-data & save sync for jk2-web.
// SPDX-License-Identifier: GPL-3.0-or-later | part of jk2-web
//
// Same-origin under jk2.virtastic.app/api/*. Two storage backends, chosen by env:
...
// Defaults sized for a real Jedi Outcast install (~1.5 GB, ~180 files) with headroom, not for hoarding.
...
// files: assets0.pk3 is ~535 MB, so the inherited 512 MiB default rejected genuine
...
const MAX_USER_BYTES  = num('MAX_USER_BYTES', 6 * 1024 * 1024 * 1024);   // per account, data + saves (a full install is ~626 MB across 4 archives)
...
// Jedi Outcast upload. idTech3 ships its data as .pk3 archives; that is the entry that matters.
let DATA_EXT_OK = /\.(pk3|cfg|dat|bin|txt|shader|bsp|md3|roq|wav|mp3|ogg|tga|jpg|jpeg|png|bmp|dds)$/i;

// ---- "Is this actually JK2 data?" ---------------------------------------------------------------
// Two tiers, because Jedi Knight II: Jedi Outcast shipped in many builds while no public hash list exists for them
...
let EDITIONS = { files: {} };
try { EDITIONS = JSON.parse(await fs.readFile(new URL('./jk2-editions.json', import.meta.url), 'utf8')); }
catch { console.warn('jk2-editions.json missing - game-data verification disabled'); }
...
function localStore() {
  ...
  return {
    ...
    // Encode each segment (JK2 filenames contain spaces) but keep the / separators; Fastify decodes
...
const setSession = (reply, uid, name) => reply.setCookie('jk2_session', jwtSign({ uid, name }), {
  httpOnly: true, secure: COOKIE_SECURE, sameSite: 'lax', path: '/' });   // no maxAge: session cookie
const currentUser = (req) => jwtVerify(req.cookies?.jk2_session);
...
app.get('/api/auth/:provider/login', async (req, reply) => {
  ...
  reply.setCookie('jk2_oauth_state', state, { httpOnly: true, secure: COOKIE_SECURE, sameSite: 'lax', path: '/', maxAge: 600 });
...
app.get('/api/auth/:provider/callback', async (req, reply) => {
  ...
  if (!code || !state || state !== req.cookies?.jk2_oauth_state) return reply.code(400).send({ error: 'bad oauth state' });
  reply.clearCookie('jk2_oauth_state', { path: '/' });
  try {
...
app.post('/api/auth/logout', async (req, reply) => { reply.clearCookie('jk2_session', { path: '/' }); return { ok: true }; });
...
app.post('/api/data/presign', async (req, reply) => {
  ...
  if (manifest) {
    ...
    for (const f of list) {
      ...
      if (VERIFY_DATA && EDITION_PATHS) {
        ...
        if (!how) return reply.code(422).send({ error: `not a recognized Jedi Knight II: Jedi Outcast data file: ${p}`, notGameData: true });
      }
  ...
  if (VERIFY_DATA && EDITION_PATHS) {
    ...
    if (!how) return reply.code(422).send({
      error: `not a recognized Jedi Knight II: Jedi Outcast data file: ${clean}`, notGameData: true });
  }
...
if (store.kind === 'local') {
  app.route({ method: ['GET', 'PUT', 'DELETE'], url: '/api/blob/*', handler: async (req, reply) => {
    ...
    if (req.method === 'PUT') {
      ...
      // cap and sends each slice with x-jk2-chunk-offset/x-jk2-total-size. Every constraint is
      ...
      const offH = req.headers['x-jk2-chunk-offset'], totH = req.headers['x-jk2-total-size'];
      if (offH !== undefined || totH !== undefined) {
        ...
        if (isData && VERIFY_DATA && EDITION_PATHS
            && !classify(clean, totalSz, req.headers['x-jk2-md5']))
          return reply.code(422).send({ error: `not a recognized Jedi Knight II: Jedi Outcast data file: ${clean}`, notGameData: true });
      ...
      if (isData && VERIFY_DATA && EDITION_PATHS
          && !classify(clean, Number(req.headers['content-length']) || 0, req.headers['x-jk2-md5']))
        return reply.code(422).send({ error: `not a recognized Jedi Knight II: Jedi Outcast data file: ${clean}`, notGameData: true });
...
app.listen({ port, host: '0.0.0.0' }).then(() =>
  app.log.info(`jk2-cloud on :${port} (base ${BASE_URL}, storage=${store.kind}${store.kind === 'local' ? ` ${DATA_DIR}` : ''}, dev-auth=${DEV_AUTH})`));
