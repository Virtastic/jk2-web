...
aws s3api put-bucket-cors --bucket "$S3_BUCKET" --cors-configuration file:///tmp/jk2-cors.json
echo "==> CORS applied:"
aws s3api get-bucket-cors --bucket "$S3_BUCKET"
rm -f /tmp/jk2-cors.json
...
echo jk2-cloud-setup-probe > /tmp/jk2-probe.txt
aws s3 cp /tmp/jk2-probe.txt "s3://$S3_BUCKET/_setup-probe.txt" >/dev/null
aws s3 cp "s3://$S3_BUCKET/_setup-probe.txt" /tmp/jk2-probe-back.txt >/dev/null
diff -q /tmp/jk2-probe.txt /tmp/jk2-probe-back.txt >/dev/null && echo "    ok: credentials can read and write"
aws s3 rm "s3://$S3_BUCKET/_setup-probe.txt" >/dev/null && echo "    ok: delete works (save sync needs it)"
rm -f /tmp/jk2-probe.txt /tmp/jk2-probe-back.txt
echo "==> bucket ready"
